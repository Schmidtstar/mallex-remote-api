...
export function ArenaScreen() {
  ...
  const getRandomPlayer = () => {
    ...
    const availablePlayers = players.filter(
      (player) => !excludedPlayers.includes(player.name),
    );
  ...
  const removePlayerFromRound = (playerName: string) => {
    ...
    announceToScreenReader(
      `${playerName} wurde aus der aktuellen Arena-Runde entfernt.`,
    );
  ...
  const addPlayerBackToRound = (playerName: string) => {
    ...
    announceToScreenReader(
      `${playerName} wurde wieder in die Arena-Runde aufgenommen.`,
    );
  };
  ...
  const getRandomTask = (categoryId: string) => {
    ...
    const hiddenTasks = hiddenTasksData
      ? new Set(JSON.parse(hiddenTasksData))
      : new Set();

    // Filter out hidden static tasks
    const staticTasks = (challenges[categoryId] || []).filter(
      (taskKey) => !hiddenTasks.has(taskKey),
    );
  ...
  const startGame = () => {
    ...
    MonitoringService.trackUserAction("arena_start", {
    ...
    try {
      ...
      announceToScreenReader(
        `Arena gestartet. Runde 1. Spieler ${player} ist an der Reihe.`,
      );
  ...
  const revealTask = () => {
    ...
    const spinInterval = setInterval(() => {
      ...
      if (spinCounter >= 8) {
        ...
        announceToScreenReader(
          `Aufgabe enthüllt: Kategorie ${revealedCategoryName}.`,
        );
      }
    }, 500);
  };
  ...
  const waitForAction = () => {
    ...
    announceToScreenReader(
      `Spieler ${selectedPlayer} wird zur Aktion aufgefordert.`,
    );
  };
  ...
  const updatePlayerPoints = async (
    ...
    try {
      ...
      console.log(
        `✅ ${playerName} erhält ${pointsToAdd} Arena-Punkte (Real-time Sync aktiv)!`,
      );

      // Announce points update
      announceToScreenReader(
        `${playerName} hat ${pointsEarned} Punkte erhalten.`,
      );
      ...
      if (updatedPlayer) {
        console.log(
          `🔍 Validation: ${playerName} now has ${updatedPlayer.arenaPoints} points`,
        );
      } else {
      ...
      alert(
        `Fehler beim Speichern der Punkte für ${playerName}. Bitte versuche es erneut.`,
      );
      announceToScreenReader(
        `Fehler beim Speichern der Punkte für ${playerName}.`,
      );
    }
  };
  ...
  const handleTaskSuccess = async () => {
    ...
    // Announce result
    announceToScreenReader(
      `${selectedPlayer} hat die Aufgabe erfolgreich abgeschlossen. Sie müssen ${sips} Schlücke ${sips === 1 ? "" : ""} trinken.`,
    );
  };
  ...
  const handleTaskFailed = async () => {
    ...
    // Announce result
    announceToScreenReader(
      `${selectedPlayer} hat die Aufgabe nicht erfolgreich abgeschlossen. Sie müssen ${sips} Schlücke ${sips === 1 ? "" : ""} trinken.`,
    );
  };
  ...
  const nextRound = () => {
    ...
    // Announce next round
    announceToScreenReader(
      `Neue Runde gestartet. Runde ${currentRound + 1}. Spieler ${player} ist an der Reihe.`,
    );
  };
        ...
        return (
          <div style={{ textAlign: "center", padding: "10px" }}>
            ...
            <div
              style={{
                background:
                  "linear-gradient(135deg, rgba(218,165,32,0.2), rgba(205,127,50,0.1))",
            ...
            <button
              ...
              disabled={
                loadingTasks ||
                players.length === 0 ||
                players.filter((p) => !excludedPlayers.includes(p.name))
                  .length === 0
              }
              style={{
        ...
        return (
          <div style={{ textAlign: "center", padding: "10px" }}>
            ...
            <div
              style={{
                background:
                  "linear-gradient(135deg, rgba(255,107,53,0.3), rgba(218,165,32,0.2))",
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: "15px",
                  border: isSpinning
                    ? "2px solid var(--ancient-gold)"
                    : "2px solid var(--ancient-bronze)",
                  transition: "all 0.3s ease",
                }}
              >
                <span
              ...
              <div
                style={{
                  ...
                  animation: isSpinning
                    ? "spin-oracle 0.5s linear infinite"
                    : "none",
                }}
              >
                {isSpinning ? "🌀" : "📜"}
        ...
        return (
          <div style={{ textAlign: "center", padding: "10px" }}>
            ...
            <div
              style={{
                background:
                  "linear-gradient(135deg, rgba(255,107,53,0.2), rgba(218,165,32,0.1))",
                padding: isMobile ? "1rem" : "2rem",
                borderRadius: "var(--radius)",
                marginBottom: "1rem",
                border: "2px solid var(--ancient-gold)",
            ...
            >
              <button
                ...
                style={{
                  ...
                  background:
                    "linear-gradient(135deg, var(--olympic-victory), #DAA520)",
                  color: "var(--ancient-night)",
